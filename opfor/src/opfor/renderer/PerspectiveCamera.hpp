...
namespace opfor {
	...
	public:
		...
		PerspectiveCamera(float aspect, float fov, float nearr, float farr)
			: _FOV(fov), _Near(nearr), _Far(farr), _Aspect(aspect), _Exposure(1.0f),
		...
		auto GetExposure() const { return _Exposure; }
		auto const &GetPosition() const { return _Position; }
		auto const &GetDirection() const { return _Direction; }
		auto const &GetProjection() const { return _Projection; }
		auto const &GetViewMatrix() const { return _View; }
		auto const &GetViewProjectionMatrix() const { return _ViewProjection; }
		auto GetYaw() const { return _Yaw; }
		auto GetPitch() const { return _Pitch; }
		auto GetRoll() const { return _Roll; }
